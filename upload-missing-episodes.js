...
// Missing episodes from test
const MISSING_EPISODES = [23, 42, 43, 44, 45, 46, 47, 50, 52, 53, 55];
...
async function main() {
  console.log('📤 Uploading Missing Episodes to Mega.nz');
  console.log('=========================================\n');
  console.log(`Uploading ${MISSING_EPISODES.length} missing episodes\n`);
  ...
  console.log(`🔐 Account: ${ACCOUNT2_EMAIL}\n`);
  ...
  for (const episodeNum of MISSING_EPISODES) {
  ...
  console.log('\n🎉 UPLOAD COMPLETE!');
  console.log('===================');
  console.log(`✅ Successful: ${successCount}/${MISSING_EPISODES.length}`);
  console.log(`❌ Failed: ${failCount}/${MISSING_EPISODES.length}\n`);
  
  if (successCount > 0) {
    console.log('✅ All 81 episodes should now work!');
