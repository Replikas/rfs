...
export default function ClientHome({ initialEpisodes, thumbnails, summaries }: ClientHomeProps) {
  ...
  return (
    <div className="pt-24 pb-12">
      ...
      <div className="px-4 md:px-12 mb-8 sticky top-20 z-40">
        <div className="max-w-md relative group">
          <div className="absolute inset-y-0 left-4 flex items-center pointer-events-none">
            <Search className="w-4 h-4 text-gray-500 group-focus-within:text-[var(--accent)] transition-colors" />
          </div>
          <input
            type="text"
            placeholder="Search episodes (e.g. Pickle, S01E01)..."
            value={searchQuery}
            onChange={(e) => setSearchQuery(e.target.value)}
            className="w-full bg-white/5 backdrop-blur-xl border border-white/10 rounded-full py-3 pl-12 pr-12 text-sm text-white focus:outline-none focus:border-[var(--accent)] focus:ring-1 focus:ring-[var(--accent)]/30 transition-all placeholder-gray-500 shadow-2xl"
          />
          {searchQuery && (
            <button 
