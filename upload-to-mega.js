...
// You'll enter your Mega.nz credentials when running the script
// Account 1: Episodes 1-40
// Account 2: Episodes 41-81
...
// Your Mega.nz credentials (enter here)
...
async function main() {
  console.log('📤 Mega.nz Upload Script for RickFlix');
  console.log('=====================================\n');

  // Check credentials
  if (ACCOUNT1_EMAIL === 'YOUR_FIRST_MEGA_EMAIL' || ACCOUNT2_EMAIL === 'YOUR_SECOND_MEGA_EMAIL') {
    console.log('❌ Please edit the script and add your Mega.nz credentials!');
    console.log('\nOpen upload-to-mega.js and replace:');
    console.log('  ACCOUNT1_EMAIL with your first Mega.nz email');
    console.log('  ACCOUNT1_PASSWORD with your first Mega.nz password');
    console.log('  ACCOUNT2_EMAIL with your second Mega.nz email');
    console.log('  ACCOUNT2_PASSWORD with your second Mega.nz password\n');
    return;
  }
  ...
  // Login to Account 1
  console.log('🔐 Logging in to Mega.nz Account 1...');
  let storage1;
  try {
    storage1 = await loginToMega(ACCOUNT1_EMAIL, ACCOUNT1_PASSWORD);
  } catch (error) {
  ...
  for (let i = 1; i <= 40; i++) {
    ...
    try {
      const result = await uploadFile(storage1, filePath, i);
      console.log(`\n   ✅ Uploaded! Embed URL ready`);
      results.push({ episodeNum: i, success: true, account: 1, ...result });
      successCount++;
    } catch (error) {
  ...
  console.log('1. Check the mega-upload-results.json file for all embed URLs');
  console.log('2. Update your video player to use these Mega.nz embeds');
  console.log('3. Videos will work on iPhone! ✅\n');
}
